...
public class ScoreProgram extends JFrame {
    ...
    // 모드 선택 라디오 버튼
    private JRadioButton autoModeBtn;
    private JRadioButton manualModeBtn;
    ...
    public ScoreProgram() {
        ...
        model = new DefaultTableModel(columns, 0) {
            ...
            public boolean isCellEditable(int row, int column) {
                return column >= 0 && column <= 3; // 점수 수정 가능
            }
        };
        ...
        model.addTableModelListener(new TableModelListener() {
            ...
            public void tableChanged(TableModelEvent e) {
                if (!autoModeBtn.isSelected()) return; // 자동모드일 때만 작동
        ...
        autoModeBtn = new JRadioButton("자동모드");
        manualModeBtn = new JRadioButton("수정모드", true);

        ButtonGroup group = new ButtonGroup();
        group.add(autoModeBtn);
        group.add(manualModeBtn);

        modePanel.add(new JLabel("성적계산 모드: "));
        modePanel.add(autoModeBtn);
        modePanel.add(manualModeBtn);

        add(modePanel, BorderLayout.WEST);

        // 모드 변경 시 버튼 활성/비활성
        autoModeBtn.addActionListener(e -> calcBtn.setEnabled(false));
        manualModeBtn.addActionListener(e -> calcBtn.setEnabled(true));
