...
class VideoProcessorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("영상 처리 프로그램 (Undo & 별도 윈도우)")

        self.image = None
        self.processed_image = None
        self.history = []  # Undo를 위한 히스토리 스택
        ...
        tk.Button(btn_frame, text="Undo", command=self.undo).grid(row=0, column=2, padx=5, pady=5)
        tk.Button(btn_frame, text="결과 별도 윈도우", command=self.show_in_new_window).grid(row=0, column=3, padx=5, pady=5)

        # 크기 조절 입력
        tk.Label(btn_frame, text="크기 비율").grid(row=1, column=0)
    ...
    def show_in_new_window(self):
        ...
        im_pil = Image.fromarray(img_rgb)
        imgtk = ImageTk.PhotoImage(image=im_pil)
        lbl = tk.Label(new_win, image=imgtk)
        lbl.imgtk = imgtk
        lbl.pack()
